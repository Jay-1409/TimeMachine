...
router.post('/generate', async (req, res) => {
  ...
  try {
    const rows = await TimeData.find({ userEmail, date })
      .select('domain totalTime category sessions')
      .lean();
      
    if (!rows.length) {
      return res.status(404).json({ error: 'No data found for that date.' });
    }
    ...
    catChart.setConfig({
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: ['#3b82f6', '#ef4444', '#8b5cf6', '#10b981', '#6b7280']
        }]
      },
      options: {
        plugins: {
    ...
    sorted.forEach(([dom, secs], i) => {
      ...
      if (i === 0) doc.rect(startX, y, totalW, rowH).fill('#ffdddd');
      else if (i === 1) doc.rect(startX, y, totalW, rowH).fill('#ffeacc');
      else if (i === 2) doc.rect(startX, y, totalW, rowH).fill('#ddffdd');
      else if (i % 2 === 0) doc.rect(startX, y, totalW, rowH).fill('#ffffff');
      else doc.rect(startX, y, totalW, rowH).fill('#f9f9f9');
    ...
    doc.moveDown(0.5);
    doc.image(catBuf, { fit: [400, 250], align: 'center' });
    doc.moveDown();
    doc.fontSize(16).text('All Sites Time Spent');
    doc.moveDown(0.5);
    doc.image(barBuf, { fit: [400, 300], align: 'center' });
