...
router.post('/send', async (req, res) => {
  ...
  try {
    ...
    const sitesHtml = Object.entries(domainTimes)
      .sort((a, b) => b[1] - a[1])
      .map(([domain, time]) => `
        <tr style="border-bottom: 1px solid #d1fae5;">
          <td style="padding: 10px; font-size: 14px; color: #1f2937;">${domain}</td>
          <td style="padding: 10px; font-size: 14px; color: #1f2937;">${formatDuration(time)}</td>
        </tr>
      `).join('');
